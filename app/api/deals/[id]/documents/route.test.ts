...
describe("GET /api/deals/:id/documents", () => {
  ...
  it("returns documents", async () => {
    ...
    const res = await GET(new Request("http://localhost/api/deals/d1/documents"), {
...
describe("POST /api/deals/:id/documents", () => {
  ...
  it("uploads a document", async () => {
    mockFindUnique.mockResolvedValueOnce({ id: "d1" });
    ...
    const res = await POST(
      new Request("http://localhost/api/deals/d1/documents", {
        method: "POST",
        body: formData,
      }),
      { params: Promise.resolve({ id: "d1" }) },
