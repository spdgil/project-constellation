...
export function StrategyDraft({ strategy: initial }: StrategyDraftProps) {
  ...
  useEffect(() => {
    ...
    async function fetchSectors() {
      try {
        const res = await fetch("/api/sectors", { signal: controller.signal });
        if (res.ok) {
  ...
  return (
    <div className="space-y-6" data-testid="strategy-draft">
      ...
      <StrategySectorLinksEditor
        linkedSectorIds={linkedSectorIds}
        sectorNameById={sectorNameById}
        moveSectorLink={moveSectorLink}
        removeSectorLink={removeSectorLink}
        sectorsLoading={sectorsLoading}
        availableSectors={availableSectors}
        unlinkedSectors={unlinkedSectors}
        sectorPickerId={sectorPickerId}
